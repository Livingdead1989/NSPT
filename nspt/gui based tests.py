...
class Ui_MainWindow(object):
    ...
    def runReport(self):
        ...
        device_dict = dict([
            ...
            ('Cisco IOS', 'cisco_ios'),
            ('Aruba OS', 'aruba_os'),
            ('Aruba OS Switch', 'aruba_osswitch'),
            ('Aruba Procurve', 'aruba_procurve'),
        ])
        login = {
        ...
        def runChecks(): 
            ...
            def checkTelnet():
                try:
                    Telnet.open(self.host_device_input.text(),'23',timeout=3) ## Timeout to prevent large hang times
                except:
            ...
            def checkExec():
                try:
                    if self.device_type_combo.currentText().lower() in hpe_devices: ## Check for HPE devices which use a different syntax
                    ...
                    return f'ERROR: connecting to device {self.host_device_input.text()} using SSH.'
                else:
                    if 'enable secret' or 'password manager' in command.lower(): ## check for keyword in command output
                        return f'PASSED: Privilege Exec mode is protected by a password on {self.host_device_input.text()}.'
                        ...
                        return f'FAILED: Privilege Exec does not have a password enabled on {self.host_device_input.text()}.'
            ...
            if self.security_test_telnet_check.isChecked():
                includeTelnet = checkTelnet() 
            if self.security_test_password_check.isChecked():
                includeExec = checkExec()
            if self.security_test_snmp_check.isChecked():  
                includeSNMP = checkSNMP()
            
            exportReport(includeTelnet, includeExec, includeSNMP)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.device_type_combo.setItemText(2, _translate("MainWindow", "HP Comware"))
        self.device_type_combo.setItemText(3, _translate("MainWindow", "Aruba OS"))
        self.device_type_combo.setItemText(4, _translate("MainWindow", "Aruba OS Switch"))
        self.device_type_combo.setItemText(5, _translate("MainWindow", "Aruba Procurve"))
